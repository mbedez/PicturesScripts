...
def check_metadata_is_null(image):
    if (image.gps_latitude is None or
        ...
        image.gps_longitude_ref is None or
        image.datetime_original is None):
        return True
    else:
        return False
...
for image_path in image_paths:
    ...
    with open(image_path, 'rb') as image_file:  # Utilisez 'rb' pour le mode lecture
        ...
        if check_metadata_is_null(my_image):
            print(f"\n{image_filename}")
            ...
            new_date = datetime(year=year, month=month, day=day, hour=0, minute=0, second=0)
